...
const AUDIENCE_SCRIPT = `(()=>{
  const slides=Array.from(document.querySelectorAll('.slide')).filter(slide=>slide.dataset.slideSkipped!=='true');
  const send=command=>parent.postMessage({type:'slides-studio:presentation-command',command},'*');
  const fit=()=>{const stage=document.querySelector('.deck-stage')||slides[0];if(!stage)return;const width=stage.offsetWidth||slides[0]?.offsetWidth||1920,height=stage.offsetHeight||slides[0]?.offsetHeight||1080,scale=Math.min(innerWidth/width,innerHeight/height);Object.assign(stage.style,{position:'absolute',left:((innerWidth-width*scale)/2)+'px',top:((innerHeight-height*scale)/2)+'px',width:width+'px',height:height+'px',transform:'scale('+scale+')',transformOrigin:'0 0'});};
  const show=(index,reduced)=>{const next=Math.max(0,Math.min(Number(index)||0,slides.length-1));document.documentElement.dataset.presentationReducedMotion=String(Boolean(reduced));slides.forEach((slide,i)=>{const active=i===next;slide.classList.toggle('active',active);slide.classList.toggle('visible',active);slide.setAttribute('aria-hidden',String(!active));slide.style.pointerEvents=active?'auto':'none';if(!active)slide.querySelectorAll('video,audio').forEach(media=>media.pause());});dispatchEvent(new CustomEvent('slides-studio:slide-change',{detail:{index:next,slideId:slides[next]?.dataset.slideId}}));fit();};
  addEventListener('message',event=>{if(event.source!==parent)return;const data=event.data;if(data?.type==='slides-studio:presentation-state'&&Number.isInteger(data.index))show(data.index,data.reducedMotion);});
  addEventListener('keydown',event=>{if(event.defaultPrevented||event.target?.closest?.('input,textarea,button,[contenteditable=true]'))return;if(['ArrowRight','ArrowDown','PageDown',' '].includes(event.key)){event.preventDefault();send('next');}else if(['ArrowLeft','ArrowUp','PageUp'].includes(event.key)){event.preventDefault();send('previous');}else if(event.key==='Home'){event.preventDefault();send('first');}else if(event.key==='End'){event.preventDefault();send('last');}else if(event.key.toLowerCase()==='f'){event.preventDefault();document.documentElement.requestFullscreen?.().catch(()=>send('fullscreen-failed'));}});
  document.body.tabIndex=-1;addEventListener('resize',fit);document.fonts?.ready?.then(fit);show(0,matchMedia('(prefers-reduced-motion: reduce)').matches);parent.postMessage({type:'slides-studio:presentation-ready'},'*');
})();`;
