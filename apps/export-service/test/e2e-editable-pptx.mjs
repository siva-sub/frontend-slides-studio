...
if (!report.limitations.some((item) => item.includes("HTML object motion") && item.includes("static frames"))) throw new Error("Object-motion limitation is missing from editable report");
if (report.nativeTransitions !== 1) throw new Error(`Native transition inventory is incomplete: ${report.nativeTransitions}`);
const transitionXml = spawnSync("python3", ["-c", "import sys,zipfile;print(zipfile.ZipFile(sys.argv[1]).read('ppt/slides/slide1.xml').decode())", output], { encoding: "utf8" });
if (transitionXml.status !== 0 || !transitionXml.stdout.includes('<p:wipe dir=\"l\"/>') || !transitionXml.stdout.includes('<a:gradFill')) throw new Error("Native transition or gradient XML is missing");
