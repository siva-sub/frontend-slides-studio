...
| Protocol | Versioned deck, diagram, media, asset, motion, transition, visual-scene, presentation-object, quality, and export-job contracts with runtime validation and backward-compatible V1 parsing |
...
| Studio editing | Sandboxed arbitrary-HTML import; Browse, Edit, and Move modes; text editing; snapping; eight-handle resizing; keyboard nudging; layers; slide reorder, duplicate, skip, include, and search; deletion; undo/redo; File System Access saving; content-hashed media replacement; and state restoration |
| Studio integration | Style and recipe browsing, typed stable-ID diagram insertion, media crop/reframe controls, asset plans and local generation jobs, object motion and transitions, current-page issue focus, and authenticated local export jobs |
...
| Agent integrations | Canonical skill and synchronized Claude, Codex, Cursor, and Claude-plugin copies for create, import/edit, assets, diagrams, motion, visual masters, validation, and export |
...
- **Editable PPTX:** named native objects plus explicit raster fallbacks and a generated quality report.
...
- `pnpm smoke:editable` verifies stable native object names, fallback inventory, fresh LibreOffice render-back when available, and the manual-review evidence gate.
